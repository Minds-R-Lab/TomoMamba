...
`mamba-ssm` and `causal-conv1d` are listed last in `requirements.txt` because
they compile CUDA kernels and need a matching toolchain.

If `mamba-ssm` will not build, `centernet_models.py` falls back to a
bidirectional GRU. That is the BiGRU baseline of Table 9, not the proposed
model, so the TomoMamba results cannot be reproduced without a working
`mamba-ssm` install.
...
It is the annotation-independent condition reported in Section 5.8. The released
uniform dump is named `per_case_uniform_v2.jsonl` for historical reasons; it is
the output of `--mode uniform`.
...
python models/stage1_screening.py
...
All reported runs use `--epochs 200 --patience 20 --seed 42`, except the four
additional seeds behind Table 6, which used `--epochs 25 --patience 8`
(Section 5.2). Note that `train_stage2.py` evaluates during training at a detection
...
| Table 6, seed stability | `seed_sweep.py` (defaults `--epochs 25 --patience 8`, the budget used for the four additional seeds; the published seed 42 run used 200 and 20, see Section 5.2) |
| Table 7, screening-threshold sweep | `end_to_end_eval.py` |
| Table 8, end-to-end on both partitions | `end_to_end_eval.py --split validation`, then `--split test` (writes `end_to_end_<split>.json`); the lesion-sensitivity row is derived from these outputs and the Stage 2 dumps |
...
the raw DICOM data. The cross-validation, gate-statistic, slice-gap and Stage 1
outputs are reproduced by running the scripts listed above.
